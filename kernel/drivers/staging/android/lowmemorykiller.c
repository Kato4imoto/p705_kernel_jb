...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
		 ...
		 * A Signal is not reached if issued condition, and lmk continously select & kill same process repeatly.
		 * Modem crash (watchdog), kernel get stuck */
#if defined (CONFIG_LGE_DEATHPENDING_LMK)
		if (test_tsk_thread_flag(p, TIF_MEMDIE) || (p->flags & PF_EXITING))
		{
			lowmem_print(2, "skip %d (%s) is terminating due to OOM killer. p->flags(%d)\n", p->pid, p->comm, p->flags);
			task_unlock(p);
			continue;
		}
#endif
